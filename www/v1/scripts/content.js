...
export function loadContent() {
  ...
  menu.addEventListener("click", (e) => {
    ...
    if (!name || !menu.contains(name)) return;

    const item = name.closest(".item");

    // Аккордеон: закрываем остальные
    menu.querySelectorAll(".item.open").forEach(openItem => {
      if (openItem !== item) {
  ...
  Object.entries(data).forEach(([category, subs]) => {
    ...
    imgCatLeft.style.setProperty("--imgCat", `url("../img/category.svg")`);
    ...
    imgCatRight.style.setProperty("--imgCat", `url("../img/category.svg")`);
...
/*
 * Рекурсивный рендер sub_1 / sub_2 / items
 */
